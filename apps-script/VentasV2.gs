...
function TEST_registrarVentaV2() {
  ...
  const prueba = {
    ...
    tipo: 'venta',
    id_cliente: '',
    notas: 'TEST VENTAS V2 - NO REAL',

    descuento_general_pct: 0,
    descuento_general_importe: 0,
...
// ============================================================
// AUXILIARES DE ACTUALIZACIÓN V2
// ============================================================
...
function TEST_cancelarVentaV2() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();

  const resultado = cancelarVentaV2_(ss, {
    id_venta: 'VTA-1787256188842',
    fecha: '2026-08-20',
    motivo: 'TEST cancelación Ventas V2'
  });
...
function PREPARAR_COLUMNAS_COBRO_PAGOS_V2() {
  const ss =
    SpreadsheetApp.getActiveSpreadsheet();

  const sh =
    ss.getSheetByName('Pagos_Venta');
  ...
  const faltantes =
    columnas.filter(function(nombre) {
      return actuales.indexOf(nombre) === -1;
    });
