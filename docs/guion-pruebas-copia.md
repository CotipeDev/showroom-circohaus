...
11. Ejecutar `tests/PruebaConciliacionEnCopia.gs`. Crea y concilia dos movimientos de prueba: uno con diferencia cero y otro con diferencia `-50`; también comprueba que un movimiento no pueda conciliarse dos veces. Deja ambos movimientos sólo en la copia.
12. Ejecutar `tests/PruebaUsuariosEnCopia.gs`. Crea un vendedor temporal, comprueba el ingreso, el cambio de contraseña propia, las restricciones del rol y la desactivación. Deja ese usuario desactivado sólo en la copia y no informa sus contraseñas en el registro.
