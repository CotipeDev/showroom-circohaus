...
async function iniciarMediosPago(){
  try{
    const[dataProcesadores,dataTarifas,dataPlanes,dataCuentas]=await Promise.all([cacheGet('getProcesadoresCobro'),cacheGet('getTarifasCobro'),cacheGet('getPlanesCuotas'),cacheGet('getCuentas')]);
    procesadoresCobroData=dataProcesadores.slice(1);
    tarifasCobroData=dataTarifas.slice(1);
    planesCuotasData=dataPlanes.slice(1);
    cuentasData=dataCuentas.slice(1);
    renderConfiguracionCobrosV2();
  }catch(e){showToast(e.message||'Error al cargar medios y planes de cobro','error')}
}
...
async function guardarPlanCobroV2(){
  try{const id=document.getElementById('cobro-edit-plan-id').value,motivo=document.getElementById('cobro-edit-plan-motivo').value.trim();if(id&&!motivo)throw new Error('Indicá brevemente el motivo del cambio.');
  const cambios={ID_Procesador:document.getElementById('cobro-edit-plan-procesador').value,Canal:document.getElementById('cobro-edit-plan-canal').value.trim()||'*',Cuotas:Number(document.getElementById('cobro-edit-plan-cuotas').value)||1,Quien_Absorbe:document.getElementById('cobro-edit-plan-absorbe').value,Costo_Negocio_Pct:Number(document.getElementById('cobro-edit-plan-costo').value)||0,Recargo_Cliente_Pct:Number(document.getElementById('cobro-edit-plan-recargo').value)||0,Monto_Minimo:Number(document.getElementById('cobro-edit-plan-minimo').value)||0,Margen_Minimo_Pct:Number(document.getElementById('cobro-edit-plan-margen').value)||0,Activo:document.getElementById('cobro-edit-plan-activo').value==='true',Notas:document.getElementById('cobro-edit-plan-notas').value.trim()};if(!cambios.ID_Procesador)throw new Error('Seleccioná un procesador.');
  if(!id&&planesCuotasData.some(p=>String(p[1])===String(cambios.ID_Procesador)&&String(p[2]||'*').toLowerCase()===String(cambios.Canal).toLowerCase()&&Number(p[4])===cambios.Cuotas))throw new Error('Ya existe un plan con ese procesador, canal y cantidad de cuotas.');
  if(id){await apiPost('editarPlanCuotas',{id_plan:id,motivo,cambios});const p=planesCuotasData.find(x=>String(x[0])===String(id));if(p){p[1]=cambios.ID_Procesador;p[2]=cambios.Canal;p[4]=cambios.Cuotas;p[5]=cambios.Costo_Negocio_Pct;p[6]=cambios.Recargo_Cliente_Pct;p[7]=cambios.Quien_Absorbe;p[8]=cambios.Monto_Minimo;p[13]=cambios.Activo;p[14]=cambios.Notas;p[15]=cambios.Margen_Minimo_Pct;}}
  else{const res=await apiPost('crearPlanCuotas',{datos:cambios});const nuevoId=res.id_plan||res.id;if(!nuevoId)throw new Error('El backend no devolvió el ID del plan.');planesCuotasData.push([nuevoId,cambios.ID_Procesador,cambios.Canal,`${cambios.Cuotas} cuota${cambios.Cuotas===1?'':'s'}`,cambios.Cuotas,cambios.Costo_Negocio_Pct,cambios.Recargo_Cliente_Pct,cambios.Quien_Absorbe,cambios.Monto_Minimo,0,'','',0,cambios.Activo,cambios.Notas,cambios.Margen_Minimo_Pct]);}
  cache.invalidar('getPlanesCuotas','getHistorialPlanesCuotas');cerrarModal('modal-editar-plan-cobro');renderConfiguracionCobrosV2();showToast(id?'Plan actualizado':'Plan creado');}catch(e){showToast(e.message||'No se pudo guardar el plan','error');}
}
